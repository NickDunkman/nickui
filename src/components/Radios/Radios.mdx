...
<Docs.FormLibraryDemos of="Radios" fieldName="pinPosition" initialValue="front">
  {({ props, error }) => (
    <Radios
      {...props}
...
<Docs.KeyboardInteractions
  keys={[
    ['Tab', 'Focuses/blurs the <ComponentName of="Radios" />'],
    [['↓', '→'], 'Moves focus to and checks the next radio'],
    [['↑', '←'], 'Moves focus to and checks the previous radio'],
