...
# How the <Docs of="Money" /> component works 🤓
...
Whether you’re filling out a banking form (withdrawal, transfer, wire, ...),
...
- “Thousands separators” (e.g. commas) are helpful for comprehending larger
...
- As an American, numbers presented in the European format can be confusing.
  `1.234,56` is 😯, `4,56` is 🤨, and `1.234` is 😰—is that a “one thousand two
...
- I like input placeholders a lot; they’re like the “self-documenting code” of
  ...
  `0.00` as a placeholder in a US$ currency input (or `0.00000000` for ₿itcoin)
  is another opportunity to create some intuition about the field type &
  importance.

### Reformatting the value _as you’re typing_ is disorienting

I’ve used currency inputs that automatically add the thousands separators _as
...
- _You can_ programatically reposition the cursor into the right place after
  ...
  shift by an unexpected amount. And now you’ve got a _ton_ of gritty code to
...
### Preventing invalid input is helpful—to an extent
...
- Blocking a second decimal point is probably ok since it’s reasonable to expect
...
- With one exception: in most situations, I think the concept of "negative
  ...
  a “negative amount to add to your account.” So <Docs of="Money" /> blocks
...
If you’re pre-filling the field with a `value` (or `defaultValue`), you should
be able to just "pass through" the data you get from any of these sources
without any intervention:

- a database (where it’s probably stored as a `float` or `decimal` type, and may
  be `null` if it’s an optional column)

- a JSON response from some API, where it’ll be a `string` or `number`, and if
...
- Within javascript, you might want to perform some calculation off of the new
  value, so you’ll need to parse it as a `Number()`. While a French user will
  intuitively understand `1.234,56` as a number, javascript won’t.
...
```ts
/** The <Money> component’s formatting-related props */
...
There is a little more nuance to the formatting options used _internally_. For
example, sometimes I need to allow the user to be "working towards" typing the
full number of decimal places, and other times, I need to display the value w/
all of the decimal places “padded on”.
 ...
 * grandchildren
 */
type MoneyFormatType = Required<MoneyFormatType, 'decimalPlaces'> & {
  /**
   * Whether to allow the user to have a partial decimal portion of the value,
   * such as a trailing decimal point, or trailing zeros that exceed the
...
When you interact with the <Docs of="Money" /> component, you’re actually
...
I don’t want the `onChange` callback to fire unless there is a _numerical_
change to the value. For example, if I progammatically update the value to
add/remove/adjust the commas, I don’t think that `<Money>`’s parent should be
...
Note: “unset” should be different than zero, so while `0`, `0.`, `0.0`, `-0.0`,
...
example, if the working input receives `"1.234,56"`, the form input’s onChange
...
`<input placeholder="0.00">`. However, as soon as you start typing, it won’t
...
directly _behind_ the working input. It uses the same styles, and I’ve "cloned"
...
### "De-formatting" on focus

Since I decided that adding/removing/moving the thousands separators (commas)
_as you type_ is too chaotic, the component removes them on focus. It does this
by overriding the internal format object to have `thousandsSeparator = ""`, and
then the updated format cascades throughout the internal dependents.

Then, when the component loses focus, it adds the thousands separators back by
...
To avoid having the internal implentation absolutely explode in complexity, it’s
...
- **`UPDATE_FORMAT`**: used to remove/add thousands separats on focus/blur, and
...
path="/blob/main/src/components/Money/Money.tsx" />, you’ll see that all of this
