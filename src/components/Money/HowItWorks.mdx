...
- **Currency inputs are often involved in high-impact use cases relative to
  other types of form controls.** Whether you’re filling out a banking form
  (withdrawal, transfer, wire, ...), e-commerce form (pricing backend, tipping),
  HR form (salaries, paycheck deductions...), or in a forecasting tool—entering
...
- **Currency formatting & internationalization is very helpful, and snazzy!**
  ...
  - “Thousands separators” (e.g. commas) are helpful for comprehending larger
    ...
    wrong amount, and fixing it first. If I ever see a comma while I’m entering
    a tip amount, I’ll be on high alert!
  - As an American, numbers presented in the European format can be confusing.
    `1.234,56` is 😯, `4,56` is 🤨, and `1.234` is 😰—is that a “one thousand
    two hundred thirty four” or have they pre-converted it for me as “one point
    two three four”? I have to imagine that users across cultures have similar
  ...
  - I like input placeholders a lot; they’re like the “self-documenting code” of
    ...
    other situations), I’m likely to interpret it as a currency amount. So
    showing `0.00` as a placeholder in a US$ currency input (or `0.00000000` for
    ₿itcoin) is another opportunity to create some intuition about the field
    type & importance.

- **_Reformatting_ the value as you type is disorienting.** I’ve used currency
  ...
  - _You can_ programatically reposition the cursor into the right place after
    ...
    but it still wasn’t a good user experience after I solved every possible
    edge case.

- **Preventing invalid input is helpful—to an extent.** For accessibility &
  ...
  blocked is frustrating unless I'm expecting it, since it’s unclear whether
  it’s blocked on purpose, or if my browser has frozen.
  - Blocking a second decimal point is probably ok since it’s reasonable to
    expect that it’s not allowed.
  ...
  - Blocking keystrokes based on having a minimum / maximum value for the field
    ...
    However, I don’t think there is a sufficiently good way to pre-educate the
    user on an allowed numeric range before they start typing, since they
    generally don’t read fine print. As a result, I’ve opted-out of having
  ...
  - With one exception: in most situations, I think the concept of "negative
    ...
    probably ask the user to a enter a “positive amount to withdraw”, rather
    than a “negative amount to add to your account.” So <Docs of="Money" />
...
- **Programatically formatting & deformatting is tedious.** In order to be as
  ...
  arrives at its front door, and where it’s going / how it’ll be used when
  ...
  - If you’re pre-filling the field with a `value` (or `defaultValue`), you
    should be able to just "pass through" the data you get from any of these
    sources, without having to transform it first:
    - a database (where it’s probably stored as a `float` or `decimal` type, and
      may be `null` if it’s an optional column)
    - a JSON response from some API, where it’ll be a `string` | `number`, and
  ...
  - When the user changes the value, there is a very high probability that you
    will need to deformat it before storing it or using it within javascript. So
    the component should deformat the new value before passing it back.
    - As far as I know, most databases do not internationalize their data types.
      Values with thousands separators or a character other than "." for the
      decimal point are rejected.
    - Within javascript, you might want to perform some calculation off of the
      new value, so you’ll need to parse it as a `Number()`. While a French user
      will intuitively understand `1.234,56` as a number, javascript won’t.
