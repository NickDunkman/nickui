...
- the "numerish value" for the form input, which is _not formatted_ and must be
...
- When the controlled `value` prop changes & it’s _numerically_ different from
...
- The controlled `value` prop can also change as a result of us calling the
  `onChange` handler, which shouldn’t clobber the working input’s "partial
  value" progress.
...
If you peak into the implementation of <Docs of="Money" />, you’ll find a hook
