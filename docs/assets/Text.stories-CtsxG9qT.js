import{r}from"./iframe-Si2UkeX9.js";import{F as D}from"./Field-DY7mIMzZ.js";import{a as O,S as h,u as F}from"./index-oXvcZMk4.js";import{r as f}from"./randomId-Dfhrkieu.js";const M=O({base:`
    w-full
    border-2 rounded-sm
    bg-white
  ...
  `,variants:{sizer:{[h.small]:"text-sm py-1.5 px-2",[h.base]:"text-base py-2.5 px-3.5",[h.large]:"text-lg py-3.5 px-4.5"},hasError:{true:"border-rose-800 focus:border-rose-800 focus:shadow-rose-300"}},defaultVariants:{sizer:h.base,hasError:!1}});function l({className:e,sizer:n,label:s,hint:o,error:a,required:w,disabled:C,id:S,"aria-describedby":z,"aria-errormessage":L,"aria-invalid":B,...V}){const[R]=r.useState(f()),H=S||(s?R:void 0),[_]=r.useState(f()),A=z||(o?_:void 0),[q]=r.useState(f()),E=L||(a&&a!==!0?q:void 0),N=F(n),k=M({sizer:N,hasError:!!a});return r.createElement(D,{className:e,sizer:n,label:s,controlId:H,hint:o,hintId:A,error:a!==!0?a:void 0,errorId:E,disabled:C,required:w},r.createElement("input",{...V,id:H,className:k,disabled:C,required:w,"aria-describedby":A,"aria-errormessage":E,"aria-invalid":B!==void 0?B:!!a}))}l.sizer=h;l.__docgenInfo={description:`A form control that allows users to enter & edit a single line of text
...
number format`},error:{required:!1,tsType:{name:"ReactNode"},description:"Error message to show along with the field, or just `true` to trigger the\nerror style without showing an error message."},required:{required:!1,tsType:{name:"boolean"},description:"When `true`, adds an asterisk to the label"},disabled:{required:!1,tsType:{name:"boolean"},description:"Prevents the user from interacting with the control"}}};const{expect:t,fn:i}=__STORYBOOK_MODULE_TEST__,U={title:"Form controls/Text",component:l},v={args:{required:!0,label:"A label",hint:"A hint",error:"An error message",defaultValue:"A problematic value",onChange:i()},play:async({canvas:e,step:n})=>{const s=e.getByLabelText("A label*"),o=e.getByTitle("required");await n("Assert accessibility of layout elements",async()=>{t(s).toHaveRole("textbox"),t(s).toHaveAccessibleDescription("A hint"),t(s).toHaveAccessibleErrorMessage("An error message"),t(o).toHaveTextContent("*")}),await n("Assert the error style",async()=>{t(s).toHaveClass("border-rose-800")})}},c={args:{placeholder:"Empty Text","aria-label":"Empty Text",onChange:i()},play:async({args:e,canvas:n,step:s,userEvent:o})=>{const a=n.getByLabelText("Empty Text");await s("Assert Text is functional without an initial value",async()=>{await o.type(a,"a"),t(a).toHaveValue("a"),t(e.onChange).toHaveBeenCalledOnce()}),await s("Reset the value",async()=>{await o.type(a,"{backspace}"),t(a).toHaveValue("")})}},d={args:{value:"Controlled Text","aria-label":"Controlled Text",onChange:i()},play:async({args:e,canvas:n,step:s,userEvent:o})=>{const a=n.getByLabelText("Controlled Text");await s("Assert `value` works",async()=>{t(a).toHaveValue(e.value)}),await s("Try adding text. `onChange` should fire, but the value is controlled, so it shouldn’t change",async()=>{await o.type(a,"a"),t(e.onChange).toHaveBeenCalledOnce(),t(a).toHaveValue(e.value)})}},p={args:{defaultValue:"Uncontrolled Text","aria-label":"Uncontrolled Text",onChange:i()},play:async({args:e,canvas:n,step:s,userEvent:o})=>{const a=n.getByLabelText("Uncontrolled Text");await s("Assert `defaultValue` works",async()=>{t(a).toHaveValue(e.defaultValue)}),await s("Typing into the Text should amend the value, since it’s uncontrolled",async()=>{await o.type(a,"a"),t(a).toHaveValue(`${e.defaultValue}a`),t(e.onChange).toHaveBeenCalledOnce()}),await s("Reset the value",async()=>{await o.type(a,"{backspace}"),t(a).toHaveValue(e.defaultValue)})}},u={args:{defaultValue:"Disabled Text",disabled:!0,"aria-label":"Disabled Text",onChange:i()},play:async({args:e,canvas:n,step:s,userEvent:o})=>{const a=n.getByLabelText("Disabled Text");await s("Assert disabled state",async()=>{t(a).toBeDisabled()}),await s("Typing should have no effect",async()=>{await o.type(a,"a"),t(e.onChange).not.toHaveBeenCalled(),t(a).toHaveValue(e.defaultValue)})}},m={args:{sizer:l.sizer.small,label:"Small label",placeholder:"Small Text",onChange:i()},play:async({canvas:e,step:n})=>{await n("Assert the small sizer style on both the Text & Field",async()=>{t(e.getByLabelText("Small label")).toHaveClass("text-sm"),t(e.getByText("Small label")).toHaveClass("text-xs")})}},g={args:{sizer:l.sizer.base,label:"Base label",placeholder:"Base Text",onChange:i()},play:async({canvas:e,step:n})=>{await n("Assert the base sizer style on both the Text & Field",async()=>{t(e.getByLabelText("Base label")).toHaveClass("text-base"),t(e.getByText("Base label")).toHaveClass("text-sm")})}},x={args:{sizer:l.sizer.large,label:"Large label",placeholder:"Large Text",onChange:i()},play:async({canvas:e,step:n})=>{await n("Assert the large sizer style on both the Text & Field",async()=>{t(e.getByLabelText("Large label")).toHaveClass("text-lg"),t(e.getByText("Large label")).toHaveClass("text-lg")})}},y={tags:["!test"],args:{sizer:[l.sizer.small,l.sizer.smBase,l.sizer.mdLarge],label:"Responsive label",placeholder:"Responsive Text",onChange:i()}},b={tags:["!dev","!test"],render:e=>r.createElement("div",{className:"flex flex-col gap-4"},r.createElement(l,{...c.args}),r.createElement(l,{...d.args}),r.createElement(l,{...p.args}),r.createElement(l,{...u.args}))},T={tags:["!dev","!test"],render:e=>r.createElement("div",{className:"flex flex-col gap-4 sm:flex-row sm:items-baseline"},r.createElement(l,{...m.args,className:"sm:flex-1"}),r.createElement(l,{...g.args,className:"sm:flex-1"}),r.createElement(l,{...x.args,className:"sm:flex-1"}),r.createElement(l,{...y.args,className:"sm:flex-1"}))};v.parameters={...v.parameters,docs:{...v.parameters?.docs,source:{originalSource:`{
  args: {
    required: true,
    label: 'A label',
    hint: 'A hint',
  ...
  play: async ({
    ...
    await step('Assert the small sizer style on both the Text & Field', async () => {
  ...
  play: async ({
    ...
    await step('Assert the large sizer style on both the Text & Field', async () => {
...
}`,...T.parameters?.docs?.source}}};const I=["FieldLayout","Empty","Controlled","Uncontrolled","Disabled","Small","Base","Large","Responsive","AllControlStates","AllSizes"],K=Object.freeze(Object.defineProperty({__proto__:null,AllControlStates:b,AllSizes:T,Base:g,Controlled:d,Disabled:u,Empty:c,FieldLayout:v,Large:x,Responsive:y,Small:m,Uncontrolled:p,__namedExportsOrder:I,default:U},Symbol.toStringTag,{value:"Module"}));export{K as S,l as T};
