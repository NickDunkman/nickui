...
I’ve used currency inputs that automatically add the thousands separators as you
...
Thankfully, `React.useReducer` offers a great way to batch complex state
changes. The reducer I wrote (wrapped in `useValueStateReducer()`) offers these
actions:
...
- `useValueState` wraps `useValueStateReducer` to simplify interaction with the
  store (`dispatch` expressions read unnaturally, IMO)
