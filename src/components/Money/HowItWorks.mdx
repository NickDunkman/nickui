...
- **Currency formatting & internationalization is very helpful, and snazzy!**
  - Currency symbols are very effective at hinting that the field is monetary
    and important
  - Currency symbols are so cool. Just look at these fellas! **฿ £ ƒ ₩ ¥ ₡ ₱ € ₫
    ₼ ៛**
  - “Thousands separators” (e.g. commas) are helpful for comprehending larger
    numbers. They may also be the difference between submitting a wildly wrong
    amount, and fixing it first. If I ever see a comma while I’m entering a tip
    amount, I’ll be on high alert!
...
- **_Reformatting_ the value as you type is disorienting.** I’ve used currency
  ...
  - _You can_ programatically reposition the cursor into the right place after
    ...
    chaotic: the existing commas can move (`1,234,567|.00` -> `12,345,678|.00`),
    and the addition/removal of commas causes the cursor to shift by an
    unexpected amount. And now you’ve got a _ton_ of gritty code to maintain,
    because there are many, many edge cases. I went down this path, but it still
    wasn’t a good user experience after I solved every possible edge case.
...
- **Preventing invalid input is helpful—to an extent.** For accessibility &
  ...
  it’s blocked on purpose, or if my browser has frozen, for example.
...
- **Programatically formatting & deformatting is tedious.**
